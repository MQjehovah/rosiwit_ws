...
void MapManager::initialize(const MapManagerConfig& config) {
    config_ = config;
    clear();
    initialized_ = true;
}

void MapManager::addPointCloud(const PointCloudPtr& cloud,
...
void MapManager::updateSubmaps(const SE3d& current_pose) {
...
Submap* MapManager::getActiveSubmap() {
    std::lock_guard<std::mutex> lock(mutex_);
...
std::vector<Submap> MapManager::getAllSubmaps() {
    std::lock_guard<std::mutex> lock(mutex_);
    return submaps_;
}

PointCloudPtr MapManager::getFullMap() {
    std::lock_guard<std::mutex> lock(mutex_);
...
PointCloudPtr MapManager::getLocalMap(const SE3d& current_pose, double radius) {
    std::lock_guard<std::mutex> lock(mutex_);
...
bool MapManager::saveToPcd(const std::string& filename) {
...
bool MapManager::loadFromPcd(const std::string& filename) {
    std::lock_guard<std::mutex> lock(mutex_);
...
bool MapManager::loadSubmaps(const std::string& path) {
    std::lock_guard<std::mutex> lock(mutex_);
...
size_t MapManager::pointCount() const {
    std::lock_guard<std::mutex> lock(mutex_);
    return total_point_count_;
}

size_t MapManager::submapCount() const {
    std::lock_guard<std::mutex> lock(mutex_);
    return submaps_.size();
}

void MapManager::optimizeMemory() {
    std::lock_guard<std::mutex> lock(mutex_);
...
bool MapManager::isInSubmapRange(const Vector3d& point, const Submap& submap) {
    double dist = (point - submap.center_pose.translation()).norm();
    return dist < config_.submap_size;
}

Submap MapManager::createNewSubmap(const SE3d& center_pose) {
    Submap submap;
...
PointCloudPtr MapManager::getVisualizationCloud(double voxel_size) {
    std::lock_guard<std::mutex> lock(mutex_);
...
bool MapManager::saveMap(const std::string& path, const std::string& format) {
    std::lock_guard<std::mutex> lock(mutex_);
...
bool MapManager::loadMap(const std::string& path, bool merge) {
    std::lock_guard<std::mutex> lock(mutex_);
...
MapMetadata MapManager::getMetadata() const {
    std::lock_guard<std::mutex> lock(mutex_);
...
MapStatistics MapManager::getStatistics() const {
    std::lock_guard<std::mutex> lock(mutex_);
