#include "utils.h"
pcl::PointCloud<pcl::PointXYZINormal>::Ptr Utils::ros2PCL(const sensor_msgs::msg::PointCloud2::SharedPtr msg, int filter_num, double min_range, double max_range)
{
    pcl::PointCloud<pcl::PointXYZINormal>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZINormal>);
    pcl::PointCloud<pcl::PointXYZI>::Ptr tmp_cloud(new pcl::PointCloud<pcl::PointXYZI>);
    pcl::fromROSMsg(*msg, *tmp_cloud);

    int point_num = tmp_cloud->size();
    ...
    for (int i = 0; i < point_num; i += filter_num)
    {
        ...
        if (p.x * p.x + p.y * p.y + p.z * p.z < min_range * min_range ||
            p.x * p.x + p.y * p.y + p.z * p.z > max_range * max_range)
            continue;
