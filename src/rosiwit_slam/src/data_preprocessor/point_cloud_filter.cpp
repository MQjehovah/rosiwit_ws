...
PointCloudFilter::PointCloudFilter(const FilterConfig& config)
    : config_(config), initialized_(true) {}

void PointCloudFilter::initialize(const FilterConfig& config) {
    config_ = config;
    initialized_ = true;
}

PointCloudPtr PointCloudFilter::process(const PointCloudPtr& cloud) {
...
PointCloudPtr PointCloudFilter::removeNaNPoints(const PointCloudPtr& cloud) {
...
PointCloudPtr PointCloudFilter::filterByRange(const PointCloudPtr& cloud) {
    PointCloudPtr filtered(new pcl::PointCloud<PointType>());
    filtered->reserve(cloud->size());
...
PointCloudPtr PointCloudFilter::filterByHeight(const PointCloudPtr& cloud) {
    PointCloudPtr filtered(new pcl::PointCloud<PointType>());
    filtered->reserve(cloud->size());
...
PointCloudPtr PointCloudFilter::voxelDownsample(const PointCloudPtr& cloud) {
    PointCloudPtr filtered(new pcl::PointCloud<PointType>());
...
PointCloudPtr PointCloudFilter::statisticalOutlierRemoval(const PointCloudPtr& cloud) {
    PointCloudPtr filtered(new pcl::PointCloud<PointType>());
...
std::vector<double> PointCloudFilter::computeCurvature(const PointCloudPtr& cloud) {
...
void PointCloudFilter::extractFeatures(const PointCloudPtr& cloud,
...
bool PointCloudFilter::motionUndistort(PointCloudPtr& cloud,
